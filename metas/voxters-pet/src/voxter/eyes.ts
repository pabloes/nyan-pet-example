...
export const createEyes = (boxter:any, variationIndex:number, color:any, texture:any) => {
    ...
    eyes.addComponent(new Transform({
      position:new Vector3(0,1/3-0.025,0.5102), 
      scale:new Vector3(-1*(1-0.05),1/3, -1)
    }));
   // mat.albedoColor= Color3.FromHexString(color);
    ...
    function applyIndex(variationIndex:number){
      eyeShape.uvs =getUvs(variationIndex+1);
    }
